...
void SendData(Junqi* pJunqi, CommHeader *header, void *data, int len)
{
    ...
    sendto(pJunqi->socket_fd, buf, length, 0, (struct sockaddr *) &pJunqi->addr,
...
#endif
    if (bind(socket_fd, (struct sockaddr *) &local, sizeof(struct sockaddr))
            < 0)
    {
        printf("Bind Error!\n");
        pthread_detach(pthread_self());
